...
export default function Page4({setImagePageData,imageDataPageData,}:{setImagePageData:React.Dispatch<React.SetStateAction<string[]>>;imageDataPageData:string[];}) {
    ...
    <div className="md:mx-10 lg:mx-20 mx-5  mb-16 mt-5">
      <section className="py-1">
        ...
        <p className="text-gray-500 mb-3 mt-1">
          Drop images to describe the job
        </p>
          ...
          <div className="flex items-center sm:space-x-4 mt-3 sm:mt-0 flex-wrap">
            {imageDataPageData.map((dataUrl: string, index: number) => {
              if(imageDataPageData[0] !== ''){
                return <Image src={dataUrl} alt={`Uploaded Image ${index}`} width={110} height={110} key={index} className="object-cover m-1 w-28 h-auto"/>
              }
            })}
          </div>
        </div>
