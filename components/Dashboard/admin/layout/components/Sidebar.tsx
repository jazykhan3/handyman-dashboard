...
export default function Sidebar({slideNav,toggleSideBar,setToggleSideBar}: {slideNav: boolean;toggleSideBar: boolean;setToggleSideBar: React.Dispatch<React.SetStateAction<boolean>>;}) {
  return (
    <div className={`bg-white shadow-lg border-2   ${toggleSideBar ? "sm:w-72 w-60" : "w-24" } fixed top-0 ${!slideNav ? "left-0" : "-left-96"} overflow-y-auto`}>
      <nav className="h-screen flex  flex-col py-20 justify-between">
        <div className={`flex flex-col justify-start items-center  ${toggleSideBar ? "sm:mx-9 mx-5" : 'justify-center pr-2' } gap-3`}>
          ...
          <SidbarNavigationLinks navigation={navigation.client} toggleSideBar={toggleSideBar} isAdmin={true}/>
