  ...
  return (
    <div className="bg-white shadow-lg px-5 md:px-20 py-6 w-full fixed z-50 left-0 right-0 top-0">
      <nav className='flex justify-between  md:justify-around items-center relative'>
        ...
        <div className='w-2/3'>
          <Link href={'/dashboard/admin/'} title='dashboard' className='text-xl font-bold  hidden md:block mb-1'>Admin Dashboard</Link>
        </div>
        <div className='flex   w-full  md:w-2/3  justify-end'>
            <div className='flex items-center justify-center'>
              <div className='relative'>
                <IoNotificationsOutline className='text-2xl cursor-pointer 'onClick={() => setNotificationToggle(!notifcationToggle)}/>
                <span className='absolute -top-4 -right-2 bg-orange text-white px-[8px] rounded-full'> {TestData.length}</span>
                {notifcationToggle && (
                <div className="absolute right-0 md:-right-6 top-8">
