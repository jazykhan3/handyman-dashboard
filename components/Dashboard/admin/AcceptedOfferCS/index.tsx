...
export default function Index() {
  ...
  return (
    <div className={`w-full ${toggleSideBar ? "lg:mx-32" : "md:mx-10"} my-12`}>
     <h1 className="text-2xl font-bold mb-7 lg:px-[9.5rem]">Accepted Offers</h1>
      <div className="flex justify-center items-center gap-14 flex-wrap">
        {TestData.map((item)=>(
          <AcceptedOfferCS key={item.id} job_title={item.job_title} listing_ID={item.listing_ID} price={item.price} posted_on={item.posted_on} status={item.status}/>
        ))}
      </div>
    </div>
  );
