...
export default function ServicePage({setServicePopUP,serviceCardData}: {setServicePopUP: React.Dispatch<React.SetStateAction<boolean>>;servicePopUp: boolean;serviceCardData?: string[];}) {
  ...
  useEffect(()=>{
    if(serviceCardData){
      try{
        const serviceKey = Object.keys(ServicesTitle).filter(item=>item.toLocaleLowerCase() === serviceCardData[1].replaceAll('-','_').toLocaleLowerCase());
  ...
  return (
    <div className="Container">
      <div className="max-w-[85rem] mx-auto  lg:mt-10 mt-5 shadow-md mb-10 bg-white rounded-md">
        {serviceCardData ? (
          <div className="flex justify-center items-center bg-orange bg-opacity-50 flex-col py-2 px-2  rounded-t-md">
            <div className="bg-white shadow flex justify-center items-center flex-col py-2 rounded-lg px-3">
              <Image src={`/ServicesIcon/${serviceCardData && serviceCardData[1] }.svg`} className="object-contain w-14 sm:w-16 md:w-[4.5rem]" alt={(route.query.slug as string) || "service"} width={70} height={70}/>
        ...
        <div className="pb-3">
          <ServicePopUpWrapper setServicePopUP={setServicePopUP} serviceCardPopUPData={serviceCardPopUPData} setSentData={setSentData} serviceTitle={serviceTitle}/>
        </div>
      </div>
    </div>
  );
